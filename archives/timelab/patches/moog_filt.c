...
void setup_this(void){
  ...
  level_lin_ctl(c_cutoff, 10.0);
  level_lin_ctl(c_res, 4.0);
  level_lin_ctl(c_output_amp, .707);
  level_lin_ctl(c_swth_freq, 0.0);
  level_lin_ctl(c_swth_thresh, .99);
  level_lin_ctl(c_swth_amp, 0.0);
  level_lin_ctl(c_noise_amp, 1.0);

  fp1 = fopen("moog_filt_out", "w");
}
