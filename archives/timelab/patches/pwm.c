...
void find_slopes(t_tlsmp freq, t_tlsmp *up, t_tlsmp *dn, t_tlsmp skew){
  ...
  t_tlsmp total_slope = 4*freq;
  if(skew>=1.0)skew = .99;
  if(skew<=0.0)skew = .01;
  *up = 1.0/skew * freq;//total_slope;
  *dn = 1.0/(1.0-skew) *freq;// * total_slope;
...
static t_tlsmp x_dot(t_tlsmp *data, t_tlsmp up, t_tlsmp dn, t_tlsmp curl, int dir){//x: triangle

  t_tlsmp out;
  //printf("data[0] %f data [1] %f up %f dn %f %d\n",data[0], data[1], up, dn, dir); 
 
  if(dir==SLP_UP)
    out = data[1] * up;//tanh_clip(data[1], curl) * up;

  else
    out = data[1] * dn;//tanh_clip(data[1], curl) * dn;
  //printf("x out %f\n", out);
  return out;

}

static t_tlsmp y_dot(t_tlsmp *data, t_tlsmp thresh, t_tlsmp flyback, int *dir){//y: square

  t_tlsmp out = 0.0;
  //printf("data[0] %f data[1] %f dir %d\n", data[0], data[1], *dir);
  ...
  for(i=0; i< s; i++)
    {
	  ...
	  //printf("%f %f %f\n", data1[0], fb1, 	  
  	  ...
  	  x->rk_children[0]->ks[j+1] = 
  	    x_dot(data0, slp_up0, slp_dn0, tcurl0, dir0);
	  
  	  x->rk_children[1]->ks[j+1] = 
  	    y_dot(data0, tri_thresh0, sqr_flyback0, &dir0);
	  
  	  /* x->rk_children[2]->ks[j+1] =  */
  	  /*   x_dot(data1, slp_up1, slp_dn1, dir1); */
	  
  	  /* x->rk_children[3]->ks[j+1] =  */
  	  /*   y_dot(data1, tri_thresh1, sqr_flyback1, &dir1); */
  ...
  for(i=0; i< s; i++)
      ...
      while(its--)
	{
      ...
      x->o_sigs[1]->s_block[i] = x->ie_children[1]->state = 
			tanh_clip(x->ie_children[1]->state, curl0);
      /* x->o_sigs[2]->s_block[i] =  */
      /* 	x->rk_children[2]->state; */
      /* x->o_sigs[3]->s_block[i] = x->rk_children[3]->state =  */
      /* 	tanh_clip(x->rk_children[3]->state, sqr_curl1); */
...
void setup_this(void){
  ...
  dwnsample->i_sigs[0] = ie_osc->o_sigs[0];
  dwnsample->i_sigs[1] = rk_osc->o_sigs[0];
