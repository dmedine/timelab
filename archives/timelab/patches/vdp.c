...
t_lin_ctl **fmdrfb_ctls;

/*****************/

void reset_oscs(void);

static t_tlsmp x_dot(t_tlsmp *data, t_tlsmp omega, t_tlsmp alpha, t_tlsmp r, t_tlsmp in, t_tlsmp fmfb){

  
  t_tlsmp out = ((omega + (fmfb * in)) *  data[1]) + (data[0]*(alpha * (1.0-r)));
  return out;

}

static t_tlsmp y_dot(t_tlsmp *data, t_tlsmp omega, t_tlsmp alpha, t_tlsmp r, t_tlsmp mu, t_tlsmp in, t_tlsmp fmfb, t_tlsmp drfb){

  t_tlsmp out;
  
  // * (1.0 - (data[0]*data[0]))
  out = (omega + (fmfb * in)) * //frequency in
    ((mu * (1.0 - (data[0] * data[0])) * data [1]) - data[0] ) + //van der pol expression
    (data[1] * (alpha * (1.0 -r))) -//circle force 
    (drfb * in);  //driver force
    //driver force
    //(mu  * (1.0 - (data[0]*data[0])) * data[1]) -
    //((omega + in) * data[0]) +
    //(data[1]*(alpha * (1.0-r)));
  return(out);
  ...
  for(i=0; i< s; i++)
    {
      ...
      fmfb0    = fmdrfb_ctls[0]->ctl_sig->s_block[i];
      fmfb1    = fmdrfb_ctls[1]->ctl_sig->s_block[i];
      fmfb2    = fmdrfb_ctls[2]->ctl_sig->s_block[i];

      drfb0    = fmdrfb_ctls[3]->ctl_sig->s_block[i];
      drfb1    = fmdrfb_ctls[4]->ctl_sig->s_block[i];
      drfb2    = fmdrfb_ctls[5]->ctl_sig->s_block[i];
	  ...
	  r2 = sqrt(data2[0]*data2[0] + data2[1]*data2[1]);
  	  ...
  	  x->rk_children[0]->ks[j+1] = 
  	    x_dot(data0, omega0, alpha0, r0, data0[0]*fb00 + data1[0]*fb01 + data2[0]*fb02, fmfb0);
	  
  	  x->rk_children[1]->ks[j+1] = 
  	    y_dot(data0, omega0, alpha0, r0, mu0,
		  data0[0]*fb00 + data1[0]*fb01 + data2[0]*fb02, fmfb0, drfb0);
	  
  	  x->rk_children[2]->ks[j+1] = 
  	    x_dot(data1, omega1, alpha1, r1, data0[0]*fb10 + data1[0]*fb11 + data2[0]*fb12, fmfb1);
	  
  	  x->rk_children[3]->ks[j+1] = 
  	    y_dot(data1, omega1, alpha1, r1, mu1,
		  data0[0]*fb10 + data1[0]*fb11 + data2[0]*fb12, fmfb1, drfb2);

  	  x->rk_children[4]->ks[j+1] = 
  	    x_dot(data2, omega2, alpha2, r2, data0[0]*fb20 + data1[0]*fb21 + data2[0]*fb22, fmfb2);
	  
  	  x->rk_children[5]->ks[j+1] = 
  	    y_dot(data2, omega2, alpha2, r2, mu2, 
		  data0[0]*fb20 + data1[0]*fb21 + data2[0]*fb22, fmfb2, drfb2);
...
void setup_this(void){
  ...
  fmdrfb_ctls = (t_lin_ctl **)malloc(sizeof(t_lin_ctl*) * 6);
  ...
  for(i=0; i<6; i++)
    fmdrfb_ctls[i] = init_lin_ctl(30+i, CTL_T_LIN, h*up);

  level_lin_ctl(fmdrfb_ctls[0], fmfb0);
  level_lin_ctl(fmdrfb_ctls[1], fmfb1);
  level_lin_ctl(fmdrfb_ctls[2], fmfb2);
  level_lin_ctl(fmdrfb_ctls[3], drfb0);
  level_lin_ctl(fmdrfb_ctls[4], drfb1);
  level_lin_ctl(fmdrfb_ctls[5], drfb2);
