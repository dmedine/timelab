...
static t_tlsmp vdp_x_dot(t_tlsmp *data, t_tlsmp alpha){
...
static t_tlsmp vdp_y_dot(t_tlsmp *data, t_tlsmp alpha, t_tlsmp beta, t_tlsmp delta, t_tlsmp gamma, t_tlsmp in){


  t_tlsmp out = alpha * 
    ((beta * ( 1.0 - (data[0] * data[0])) * data[1]) - data[0]);
  ...
  for(i=0; i< s; i++)
    {
	  ...
	  //printf("%f %f %f\n", data1[0], fb1, 	  
  	  ...
  	  x->rk_children[2]->ks[j+1] = 
  	    vdp_x_dot(data1, alpha0);
	  
  	  x->rk_children[3]->ks[j+1] = 
  	    vdp_y_dot(data1, alpha0, beta0, delta0, gamma0, data0[0]);
...
void setup_this(void){
  ...
  dwnsample->i_sigs[1] = rk_osc->o_sigs[2];
