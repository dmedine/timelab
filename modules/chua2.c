...
static tl_smp x_dot(tl_UDS_node *x, int iter){
  ...
  /* out = phi_piecewise(solver->inlets[0]->smps[iter]) */
  /*   +alpha */
  /*   * (*x->data_in[1]            // y */
  /*      - *x->data_in[0]          // - x */
  ...
  out = alpha * (*x->data_in[1] + phi_piecewise(solver->inlets[0]->smps[iter]));// - *x->data_in[1;]
...
static tl_smp stage(tl_UDS_node *x, int iter) {
  ...
  out = beta * (phi_piecewise(*x->data_in[1]) + phi_piecewise(*x->data_in[0]));// + phi_piecewise(solver->inlets[0]->smps[iter]));// - *x->data_in[1];
...
// init func
void tl_init_chua2(tl_arglist *args){
  ...
  solver = tl_init_UDS_solver(2, 2, 1);
  ...
  stage_node = tl_init_UDS_node(stage, 2, 1);
  tl_reset_UDS_node(stage_node, 0.0);
  tl_push_UDS_node(solver->UDS_net, stage_node);

  // hook up the inlets and outlets
  // x_node in[0] = x, in[1] = y
  x_node->data_in[0] = x_node->data_out;
  x_node->data_in[1] = stage_node->data_out;
  
  // y_node in[0] = x, in[1] = y, in[2] = z
  stage_node->data_in[0] = x_node->data_out;
  stage_node->data_in[1] = stage_node->data_out;
...
// control interface
tl_ctl *tl_reveal_ctls_chua2(void){return chua_head;}

// kill func
void tl_kill_chua2(tl_class *class_ptr){
...
// dsp func
void tl_dsp_chua2(int samples, void *mod_ptr){
