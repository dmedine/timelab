...
typedef struct _UDS_osc{
  ...
  tl_ctl *k_alpha; // duffing equation variables
  tl_ctl *k_beta;
...
static tl_smp osc_sin(tl_UDS_node *x, int iter){
  ...
  tl_smp alpha; // duffing equation variable
  tl_smp beta;  // duffing equation variable
  tl_smp gamma[osc_cnt]; // duffing eqaution varaible
  ...
  alpha = y->k_alpha->outlet->smps[iter];
  beta = y->k_beta->outlet->smps[iter];
  ...
  // normal regime
    ...
    out = -1*(omega + lambda_n) 
      * *x->data_in[which*2+1]
      - alpha * *x->data_in[which*2+1]
      - beta * *x->data_in[which*2+1] * *x->data_in[which*2+1] * *x->data_in[which*2+1]
      //- gamma * *x->data_in[which*2+1]
...
static tl_smp osc_cos(tl_UDS_node *x, int iter){
  ...
  if(sync==1)
    ...
    out = (omega + lambda_n)
      * *x->data_in[which*2];
      /* - alpha * *x->data_in[which*2] */
      /* - beta * *x->data_in[which*2] * *x->data_in[which*2] * *x->data_in[which*2] */
      /* - gamma * *x->data_in[which*2] */
...
// control interface
tl_ctl *tl_reveal_ctls_fm_duff(void){
...
// kill func
void tl_kill_fm_duff(tl_class *class_ptr){
...
// dsp func
void tl_dsp_fm_duff(int samples, void *mod_ptr){
