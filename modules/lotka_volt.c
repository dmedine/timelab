...
static tl_smp prey_dot(tl_UDS_node *x, int iter){
  ...
  tl_smp out; // output
  tl_smp alpha = k_alpha->outlet->smps[iter];
  tl_smp beta  = k_beta->outlet->smps[iter]; 
  tl_smp m0    = k_m0->outlet->smps[iter];
  tl_smp m1    = k_m1->outlet->smps[iter];


  /* printf("%f\n", beta * *x->data_in[0] * *x->data_in[1]); */
  /* out = alpha * phi_cubic(*x->data_in[1]) */
  /* - beta * *x->data_in[0] * *x->data_in[1]; */

  out = alpha * *x->data_in[0]
    - beta * *x->data_in[0] * *x->data_in[1]; 
...
static tl_smp pred_dot(tl_UDS_node *x, int iter){
  ...
  tl_smp out;
  tl_smp delta = k_delta->outlet->smps[iter];
  tl_smp gamma = k_gamma->outlet->smps[iter]; 
  tl_smp m0    = k_m0->outlet->smps[iter];
  tl_smp m1    = k_m1->outlet->smps[iter];

  //printf("%f\n", delta * *x->data_in[0] * *x->data_in[1]);
  /* out = delta * *x->data_in[0] * *x->data_in[1] */
  /*   - gamma * phi_cubic(*x->data_in[0]);  */
