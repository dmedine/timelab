...
tl_smp vel_dot(tl_UDS_node *x, int iter){
  ...
  out =  // positional difference between this and the left mass
    -1.0 * y->k_Kl->outlet->smps[iter] * (tanh(*x->data_in[0]) - tanh(*x->data_in[1])) - // positional difference between this and the right mass
    y->k_Kr->outlet->smps[iter] *(tanh(*x->data_in[0]) - tanh(*x->data_in[2])) -
...
void tl_init_string(tl_arglist *args){
...
tl_ctl *tl_reveal_ctls_string(void){
  return ctl_head;
}

void tl_kill_string(tl_class *class_ptr){
...
void tl_dsp_string(int samples, void *mod_ptr){
