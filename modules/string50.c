...
tl_smp vel_dot(tl_UDS_node *x, int iter){
  ...
  // no need to compute bow model if none
  if(y->k_barea->outlet->smps[iter] != 0 &
     k_bvel->outlet->smps[iter]!=0 &
     k_bforce->outlet->smps[iter]!=0 &
     k_bfric->outlet->smps!=0)
  ...
  out =  // positional difference between this and the left mass
    -1.0 * y->k_Kl->outlet->smps[iter] * (*x->data_in[0] - *x->data_in[1]) - // positional difference between this and the right mass
    y->k_Kr->outlet->smps[iter] *(*x->data_in[0] - *x->data_in[2]) -
...
void tl_init_string50(tl_arglist *args){
...
tl_ctl *tl_reveal_ctls_string50(void){
  return ctl_head;
}

void tl_kill_string50(tl_class *class_ptr){
...
void tl_dsp_string50(int samples, void *mod_ptr){
